...
- **No release before v0.1.0 krush.** The first usable bar is the pad MVP: load tracks, loop pads auto-locked to one tempo, whip/wiki scratch pads, **record pads into a master timeline with launch quantization (always on-beat)**, and render a mix to WAV. (The two-deck model was retired in the 2026-06-07 pivot, and the step-grid arrange was superseded by the looper model the same day — see `.am/inception.md`.) Nothing is tagged before that story is delivered and accepted.
