...
- Init subscriber in `main.rs`: human-friendly to stderr (off by default), JSON to `~/.bigpoppa/log/` when `--log` flag set.
- Custom panic hook flushes the log and writes a one-line crash summary.
- No `println!` outside of CLI subcommands.

## Acceptance

- [ ] `big-poppa --log` produces a JSON log file under `~/.bigpoppa/log/`.
